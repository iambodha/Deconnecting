...
def geonames_query(query, country_code, total_done=0):
    ...
    while True:
        ...
        if response.status_code != 200:
            print(
                f"Error: {response.status_code} - Error occurred while querying GeoNames API."
            )
    ...
    filtered_data = [
        entry for entry in all_responses if entry.get("fcodeName") == original_query
    ]
...
def get_country_locations(country_code, country):
    ...
    for order_1 in tqdm(
        query_result_1, desc="Processing first-order divisions", unit="division"
    ):
        order_1_data = {
        ...
        query_result_2 = geonames_query(
            f'second-order administrative division {order_1.get("name")}', country_code
        )
        ...
        for order_2 in tqdm(
            query_result_2,
            desc="Processing second-order divisions",
            unit="division",
            leave=False,
        ):
            order_2_data = {
            ...
            query_result_3 = geonames_query(
                f'third-order administrative division {order_2.get("name")}',
                country_code,
            )
            ...
            for order_3 in tqdm(
                query_result_3,
                desc="Processing third-order divisions",
                unit="division",
                leave=False,
            ):
                order_3_data = {
                ...
                query_result_4 = geonames_query(
                    f'fourth-order administrative division {order_3.get("name")}',
                    country_code,
                )
    ...
    flatten_and_write_to_csv(
        result_json, country, f"../allData/{country}_locations.csv"
    )


def flatten_and_write_to_csv(json_data, country_name, csv_file_path):
    def flatten_json(
        json_data, first_order="", second_order="", third_order="", result=[]
    ):
    ...
    print(
        f"Number of unique locations after removing duplicates: {len(flattened_data_no_duplicates)}"
    )
